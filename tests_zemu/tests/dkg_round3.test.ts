...
describe.each(models)('dkg_round3', function (m) {
  ...
  describe.each([{ p: 4, min: 2 }])('participants', function ({ p: participants, min: minSigners }) {
    it('p: ' + participants + ' - min: ' + minSigners, async function () {
      ...
      for (let i = 0; i < globalSims.length; i++) await globalSims[i].start({ ...defaultOptions, model: m.name })
      ...
      try {
        ...
        for (let i = 0; i < participants; i++) {
          const round2 = await runMethod(globalSims, i, async (app: IronfishApp) => {
            const round2 = await app.dkgRound2(
              PATH,
              i,
          ...
          if (!round2.publicPackage || !round2.secretPackage) throw new Error('no round 1 found')
